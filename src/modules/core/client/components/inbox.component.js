...
import { getCampaigns } from '../../../marketing/campaign/client/campaign.actions';
...
export default function Inbox() {
    ...
    useEffect(() => {
        ...
        dispatch(getCampaigns('?limit=5&status=schedule'))
    }, []);


    return (
        <div className={`shadow-sm bg-white mb-3 cdp-inbox ${show ? "cdp-inbox__expand" : ""}`}>
            <h5 className={window.innerWidth<576 ? "fs-6 p-sm-3 py-3 px-3 cdp-text-primary fw-bold mb-0 d-flex justify-content-between cdp-inbox__header":
            "p-sm-3 py-3 px-4 cdp-text-primary fw-bold mb-0 d-flex justify-content-between cdp-inbox__header"}>
                ...
                <Tabs showArrowButton={true} showModalButton={false}>
                    ...
                    <PanelList>
                            ...
                            {!isUnauthorized
                                ? <div className="cdp-inbox__tab-detail">
                                    {hcps.users !== undefined && hcps.users.length < 6 &&
                                        hcps.users.map((user, key) => <ul key={key} className="cdp-inbox__list p-0 m-0">
                                            <li key={key} className="cdp-inbox__list-item d-flex justify-content-between  align-items-center border-bottom p-2 p-lg-3">
                                                <span className="cdp-inbox__list-item-col large cdp-text-primary fw-bold text-break">{user.email}</span>
                                                <span className="cdp-inbox__list-item-col cdp-text-primary fw-bold px-3 text-break">{new Date(user.created_at).toLocaleDateString('en-GB', {
                                    ...
                                    {hcps.users !== undefined && hcps.users.length !== 0 ?
                                        <Link to={{ pathname: "/information/list/cdp", state: { filterSetting } }} className="d-inline-block p-2 p-lg-3 text-uppercase cdp-text-secondary active small fw-bold">
                                            {hcps.total > 5 && 'More Pending'}
                                        </Link>
                                        : <h5 className="d-block py-5 px-2 text-uppercase text-center mb-0"><i className="far fa-clock me-2 cdp-text-secondary"></i>No data found</h5>
                                    }
                                </div>
                                : <div className="cdp-inbox__tab-detail">
                                    <span className="d-block py-5 px-2 text-uppercase text-center mb-0"><i className="fas fa-ban me-2 cdp-text-secondary"></i>You do not have permission to approve HCP users.</span>
                                </div>
                            }
                        </Panel>
                                ...
                                <div className="cdp-inbox__tab-detail">
                                    {partners !== null && partners.length !== 0 ? partners.map((partner, key) => {
                                        return <ul key={key} className="cdp-inbox__list p-0 m-0">
                                            <li key={key} className="cdp-inbox__list-item d-flex justify-content-between  align-items-center border-bottom p-2 p-lg-3">
                                                <span className="cdp-inbox__list-item-col large cdp-text-primary fw-bold text-break">{partner.email}</span>
                                                <span className="cdp-inbox__list-item-col cdp-text-primary fw-bold px-3 text-break">{new Date(partner.created_at).toLocaleDateString('en-GB', {
                                                    day: 'numeric', month: 'short', year: 'numeric'
                                                }).replace(/ /g, ' ')}</span>
                                                <span className="cdp-inbox__list-item-col small text-break">
                                    ...
                                    {totalPartners > 5 &&
                                        <Link to={{ pathname: `/business-partner/partner-management/${partners[0].entity_type}s` }} className="d-inline-block p-2 p-lg-3 text-uppercase cdp-text-secondary active small fw-bold">
                                            More Pending
                                        </Link>
                                    }
                                </div>}
                        ...
                        <Panel>
                            {
                                <div className="cdp-inbox__tab-detail">
                                    {campaignList && campaignList.campaigns && campaignList.campaigns.length ?
                                        campaignList.campaigns.map((campaign, key) =>
                                        (<ul key={key} className="cdp-inbox__list p-0 m-0">
                                            <li key={key} className="cdp-inbox__list-item d-flex justify-content-between  align-items-center border-bottom p-2 p-lg-3">
                                                <span className="cdp-inbox__list-item-col large cdp-text-primary fw-bold text-break">{campaign.title}</span>
                                                <span className="cdp-inbox__list-item-col large cdp-text-primary px-3 text-break">
                                                    Scheduled on <span className="fw-bold">{new Date(campaign.sendTime).toLocaleTimeString('en-GB', {
                                    ...
                                    {campaignList && campaignList.metadata && campaignList.metadata.total > 5 &&
                                        <Link to={{ pathname: '/marketing/manage-campaigns', state: { status: 'schedule' } }} className="d-inline-block p-2 p-lg-3 text-uppercase cdp-text-secondary active small fw-bold">
                                            More scheduled
                                        </Link>
                                    }
                                </div>
