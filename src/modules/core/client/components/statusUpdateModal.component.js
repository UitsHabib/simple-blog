...
export default function statusupdateModal(props) {
    ...
    const { user, show, onHide, type, onStatusUpdate } = props;
        ...
        >
            ...
            <Modal.Body>
                <div className="p-3">
                    <div className="row">
                        <div className="col">
                            <h4 className="fw-bold">{`${consentForUser.first_name} ${consentForUser.last_name}`}</h4>
                    ...
                    <div>
                        <h5 className="fw-bold my-3">Consents: </h5>
                                ...
                                {consentForUser.consents && consentForUser.consents.length ?
                                    consentForUser.consents.map(consent => {
                                        return <div className="pb-1" key={consent.id} >
                                            <i className={`icon ${consent.consent_given ? 'fas fa-check cdp-text-primary me-3' : 'fas fa-check-double cdp-text-primary me-3'} cdp-text-primary me-2 small`}></i>{consent.classification}
                                        </div>
                                    })
                    ...
                    <Formik
                        ...
                        onSubmit={(values, actions) => {
                            if (values.selectedStatus === 'approve') {
                                if (values.comment === 'other') values.comment = values.other_comment.trim();
                        ...
                        {formikProps => (
                            <Form onSubmit={formikProps.handleSubmit}>
                                <div className="row">
                                    <div className="col-6">
                                        <a
                                            className={`btn btn-block cdp-btn-outline-primary mt-4 p-2 fw-bold ${formikProps.values.selectedStatus === 'approve' ? 'selected' : ''}`}
                                            onClick={() => {
                                    ...
                                    <div className="col-6">
                                        <a
                                            ...
                                            className={`btn btn-block cdp-btn-outline-danger mt-4 p-2 fw-bold  ${formikProps.values.selectedStatus === 'reject' ? 'selected' : ''}`}
                                        >
                                            Reject User
                                        </a>
                                    </div>
                                ...
                                {formikProps.values.selectedStatus === 'approve' && <div className="row mt-4">
                                    <div className="col-12 col-sm-12">
                                        <div className="form-group mb-0">
                                            <label className="form-label fw-bold" htmlFor="comment">Comment <span className="text-danger">*</span></label>
                                            <div className="form-check custom-radio pb-2">
                                                <Field className="form-check-input" id="UUIDmanually" data-testid='comment' type="radio" name="comment" value="HCP User did a mistake in typing UUID manually" />
                                                <label className="form-check-label fw-bold" for="UUIDmanually"> HCP User did a mistake in typing UUID manually</label>
                                            </div>
                                            <div className="form-check custom-radio pb-2">
                                                <Field className="form-check-input" data-testid='comment' type="radio" id="PharmaCompanies" name="comment" value="HCP User has exclusivity with other Pharma Companies" />
                                                <label className="form-check-label fw-bold" for="PharmaCompanies">HCP User has exclusivity with other Pharma Companies</label>
                                            </div>
                                            <div className="form-check custom-radio pb-2">
                                                <Field className="form-check-input" data-testid='comment' id="OneKeypopulation" type="radio" name="comment" value="HCP User is not in the customers IQVia OneKey population" />
                                                <label className="form-check-label fw-bold" for="OneKeypopulation">HCP User is not in the customers IQVia OneKey population</label>
                                            </div>
                                            <div className="form-check custom-radio pb-2">
                                                <Field className="form-check-input" data-testid='comment' id="Other" type="radio" rows="4" name="comment" value="other" />
                                                <label className="form-check-label fw-bold" for="Other">Other:</label>
                                            </div>
                                            <div>
